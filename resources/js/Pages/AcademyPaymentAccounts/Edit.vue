      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-6">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Payment Account</h1>
              <p class="mt-1 text-sm text-gray-500">Account ki details update karo</p>
            </div>
        ...
        <div class="max-w-2xl">
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-4">
            <h2 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Payment Method</h2>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">

              <button type="button" @click="selectMethod('cash')"
                :class="form.payment_method === 'cash' ? 'border-2 border-green-500 bg-green-50 ring-2 ring-green-200' : 'border-2 border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
              ...
              <button type="button" @click="selectMethod('bank_transfer')"
                :class="form.payment_method === 'bank_transfer' ? 'border-2 border-blue-500 bg-blue-50 ring-2 ring-blue-200' : 'border-2 border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                class="relative flex flex-col items-center justify-center p-4 rounded-xl transition-all cursor-pointer">
              ...
              <button type="button" @click="selectMethod('jazzcash')"
                :class="form.payment_method === 'jazzcash' ? 'border-2 border-red-500 bg-red-50 ring-2 ring-red-200' : 'border-2 border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                class="relative flex flex-col items-center justify-center p-4 rounded-xl transition-all cursor-pointer">
              ...
              <button type="button" @click="selectMethod('easypaisa')"
                :class="form.payment_method === 'easypaisa' ? 'border-2 border-emerald-500 bg-emerald-50 ring-2 ring-emerald-200' : 'border-2 border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50'"
                class="relative flex flex-col items-center justify-center p-4 rounded-xl transition-all cursor-pointer">
            ...
            <div v-if="form.payment_method === 'bank_transfer'" class="bg-white rounded-xl shadow-sm border border-blue-200 p-5 mb-4">
              <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p class="text-xs text-gray-500">Bank account ki details</p>
                </div>
              </div>
              ...
              <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div class="flex gap-3 w-full sm:w-auto">
                  ...
                  <Button type="submit" variant="primary" :loading="form.processing" class="flex-1 sm:flex-none text-sm">
                    <span v-if="!form.processing">Update Account</span>
...
const form = useForm({
  account_title:  props.account.account_title ?? '',
  payment_method: props.account.payment_method ?? '',
  ...
  instructions:   props.account.instructions ?? '',
  is_active:      props.account.is_active ?? true,
})

const selectMethod = (method) => {
  if (form.payment_method === method) return
...
const submit = () => {
  form.put(route('academy-payment-accounts.update', props.account.id), { preserveScroll: true })
}
</script>
