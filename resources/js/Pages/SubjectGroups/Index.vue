<template>
  <AppLayout>
    <div class="min-h-screen flex flex-col">
      <!-- Main Content -->
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        
        <!-- Page Header -->
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Subject Groups Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage curriculum subject groups</p>
            </div>
          </div>
        </div>

        <!-- Success/Error Messages -->
        <div v-if="showSuccessMessage && $page.props.flash.success" class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
              </svg>
              {{ $page.props.flash.success }}
            </div>
            <button @click="showSuccessMessage = false" class="text-green-700 hover:text-green-900">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
              </svg>
            </button>
          </div>
        </div>
        <div v-if="showErrorMessage && $page.props.flash.error" class="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative">
          <div class="flex items-center justify-between">
            <div class="flex items-center">
              <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/>
              </svg>
              {{ $page.props.flash.error }}
            </div>
            <button @click="showErrorMessage = false" class="text-red-700 hover:text-red-900">
              <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
              </svg>
            </button>
          </div>
        </div>

        <!-- Create/Edit Form -->
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="mb-4">
            <h2 class="text-lg sm:text-xl font-semibold text-gray-800">
              {{ editMode ? 'Edit Subject Group' : 'Create New Subject Group' }}
            </h2>
          </div>

          <form @submit.prevent="submit">
            <div class="grid grid-cols-2 gap-4 sm:gap-6">
              <!-- Group Name -->
              <div>
                <label for="group_name" class="block text-sm font-medium text-gray-700 mb-2">
                  Group Name <span class="text-red-500">*</span>
                </label>
                <input
                  id="group_name"
                  v-model="form.group_name"
                  type="text"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                  :class="{ 'border-red-500': form.errors.group_name }"
                  placeholder="e.g., Science Group, Arts Group"
                  required
                />
                <p v-if="form.errors.group_name" class="mt-1 text-sm text-red-600">
                  {{ form.errors.group_name }}
                </p>
              </div>

              <!-- Description -->
              <div>
                <label for="description" class="block text-sm font-medium text-gray-700 mb-2">
                  Description
                </label>
                <textarea
                  id="description"
                  v-model="form.description"
                  rows="3"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                  :class="{ 'border-red-500': form.errors.description }"
                  placeholder="Brief description of this subject group..."
                ></textarea>
                <p v-if="form.errors.description" class="mt-1 text-sm text-red-600">
                  {{ form.errors.description }}
                </p>
              </div>

              <!-- Subject Selection -->
              <div>
                <label for="subject_ids" class="block text-sm font-medium text-gray-700 mb-2">
                  Select Subjects
                </label>
                
                <div v-if="loadingSubjects" class="p-4 text-center text-gray-500 border border-gray-300 rounded-md">
                  Loading subjects...
                </div>
                
                <select
                  v-else
                  id="subject_ids"
                  v-model="form.subject_ids"
                  multiple
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                  :class="{ 'border-red-500': form.errors.subject_ids }"
                  size="10"
                >
                  <optgroup v-if="compulsorySubjects.length > 0" label="Compulsory Subjects">
                    <option
                      v-for="subject in compulsorySubjects"
                      :key="'comp-' + subject.id"
                      :value="subject.id"
                    >
                      {{ subject.subject_name }}{{ subject.subject_code ? ' (' + subject.subject_code + ')' : '' }}
                    </option>
                  </optgroup>
                  
                  <optgroup v-if="optionalSubjects.length > 0" label="Optional Subjects">
                    <option
                      v-for="subject in optionalSubjects"
                      :key="'opt-' + subject.id"
                      :value="subject.id"
                    >
                      {{ subject.subject_name }}{{ subject.subject_code ? ' (' + subject.subject_code + ')' : '' }}
                    </option>
                  </optgroup>
                </select>
                
                <p class="mt-1 text-xs text-gray-500">Hold Ctrl (Windows) or Command (Mac) to select multiple subjects</p>
                
                <!-- Selected Subjects Display -->
                <div v-if="form.subject_ids.length > 0" class="mt-3 flex flex-wrap gap-2">
                  <span
                    v-for="subjectId in form.subject_ids"
                    :key="'selected-' + subjectId"
                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800"
                  >
                    {{ getSubjectName(subjectId) }}
                    <button
                      type="button"
                      @click="removeSubject(subjectId)"
                      class="ml-2 inline-flex items-center justify-center w-4 h-4 text-indigo-600 hover:text-indigo-800"
                    >
                      <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                      </svg>
                    </button>
                  </span>
                </div>
                
                <p v-if="form.errors.subject_ids" class="mt-1 text-sm text-red-600">
                  {{ form.errors.subject_ids }}
                </p>
              </div>

              <!-- Status -->
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Status
                </label>
                <div class="flex items-center mt-3">
                  <input
                    id="is_active"
                    v-model="form.is_active"
                    type="checkbox"
                    class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                  />
                  <label for="is_active" class="ml-2 block text-sm text-gray-900">
                    Active
                  </label>
                </div>
                <p v-if="form.errors.is_active" class="mt-1 text-sm text-red-600">
                  {{ form.errors.is_active }}
                </p>
              </div>
            </div>

            <!-- Submit Buttons -->
            <div class="mt-6 flex items-center justify-end gap-3 sm:gap-4">
              <Button 
                v-if="editMode"
                type="button"
                variant="secondary"
                @click="cancelEdit"
                class="w-full sm:w-auto shadow-sm hover:shadow-md transition-all text-sm"
              >
                Cancel
              </Button>
              <Button 
                type="submit"
                variant="primary"
                :loading="form.processing"
                class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm"
              >
                <span v-if="!form.processing">{{ editMode ? 'Update Group' : 'Create Group' }}</span>
                <span v-else>{{ editMode ? 'Updating...' : 'Creating...' }}</span>
              </Button>
            </div>
          </form>
        </div>

        <!-- Filters Card -->
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <div>
              <Input
                v-model="filters.search"
                placeholder="Search groups..."
                @input="searchDebounced"
                class="w-full text-sm"
              />
            </div>

            <div>
              <select
                v-model="filters.is_active"
                @change="loadData"
                class="w-full px-3 sm:px-4 py-2 text-sm border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="">All Status</option>
                <option value="1">Active</option>
                <option value="0">Inactive</option>
              </select>
            </div>

            <Button 
              variant="secondary" 
              @click="resetFilters"
              class="w-full sm:w-auto shadow-sm hover:shadow-md transition-all duration-200 text-sm"
            >
              Reset Filters
            </Button>
          </div>
        </div>

        <!-- Desktop/Tablet Table View -->
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          <!-- Table Header with Search -->
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 bg-gray-50 gap-3">
            <div class="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              <span class="text-xs sm:text-sm text-gray-700">Show</span>
              <select 
                v-model="perPage" 
                @change="changePerPage"
                class="px-3 sm:px-6 py-1.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs sm:text-sm"
              >
                <option value="10">10</option>
                <option value="25">25</option>
                <option value="50">50</option>
                <option value="100">100</option>
              </select>
              <span class="text-xs sm:text-sm text-gray-700">entries</span>
            </div>

            <div class="w-full sm:w-64">
              <div class="relative">
                <input
                  v-model="tableSearch"
                  @input="tableSearchDebounced"
                  type="text"
                  placeholder="Search in table..."
                  class="w-full pl-9 sm:pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs sm:text-sm"
                />
                <svg class="absolute left-2.5 sm:left-3 top-2.5 h-4 w-4 sm:h-5 sm:w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                </svg>
              </div>
            </div>
          </div>

          <!-- Table -->
          <div class="overflow-x-auto">
            <table id="subject-groups-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    #
                  </th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    Group Name
                  </th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    Description
                  </th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    Subjects
                  </th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    Status
                  </th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody class="bg-white text-center divide-y divide-gray-100">
                <!-- DataTables will populate this -->
              </tbody>
            </table>
          </div>

          <!-- Table Footer -->
          <div class="flex flex-col sm:flex-row items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 bg-gray-50 gap-3 sm:gap-4">
            <div class="text-xs sm:text-sm text-gray-600" id="table-info">
              <!-- Info will be inserted here -->
            </div>
            <div id="table-pagination">
              <!-- Pagination will be inserted here -->
            </div>
          </div>
        </div>

        <!-- Mobile Card View -->
        <div class="md:hidden space-y-3 sm:space-y-4">
          <!-- Loading State -->
          <div v-if="mobileLoading" class="flex items-center justify-center py-12 bg-white rounded-lg shadow">
            <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div>
          </div>
          
          <!-- Empty State -->
          <div v-else-if="mobileGroups.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No subject groups found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Group Cards -->
          <div v-else v-for="(group, index) in mobileGroups" :key="group.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-xs font-semibold text-gray-500">#{{ mobileOffset + index + 1 }}</span>
                    <h3 class="text-base font-semibold text-gray-900">{{ group.group_name }}</h3>
                  </div>
                  <p v-if="group.description" class="text-xs text-gray-500 mt-1">{{ group.description }}</p>
                </div>
                <span :class="getStatusClass(group.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
                  {{ group.is_active ? 'Active' : 'Inactive' }}
                </span>
              </div>

              <!-- Subjects Count Badge -->
              <div class="mb-3">
                <div class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mb-1">
                  {{ getSubjectCount(group) }} Total Subjects
                </div>
                <div class="flex gap-2 text-xs mt-1">
                  <span class="text-purple-600">{{ getCompulsoryCount(group) }} Compulsory</span>
                  <span class="text-gray-400">|</span>
                  <span class="text-gray-600">{{ getOptionalCount(group) }} Optional</span>
                </div>
              </div>

              <!-- Actions -->
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button 
                  @click="viewGroupSubjectsMobile(group.id)"
                  class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors flex items-center justify-center gap-1"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                  </svg>
                  View
                </button>
                <button 
                  @click="editSubjectGroupMobile(group)"
                  class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors flex items-center justify-center gap-1"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                  </svg>
                  Edit
                </button>
                <button 
                  @click="() => { groupToDelete = group.id; showDeleteModal = true; }"
                  class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors flex items-center justify-center gap-1"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                  </svg>
                  Delete
                </button>
              </div>
            </div>
          </div>
        </div>

        <!-- Mobile Pagination -->
        <div v-if="mobileGroups.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            <button 
              @click="prevPage"
              :disabled="mobileCurrentPage === 1 || mobileLoading"
              class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed bg-white hover:bg-gray-50 transition-colors flex items-center gap-1"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
              </svg>
              Previous
            </button>
            
            <div class="text-center">
              <div class="text-sm font-medium text-gray-900">Page {{ mobileCurrentPage }} of {{ mobileTotalPages }}</div>
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total groups</div>
            </div>
            
            <button 
              @click="nextPage"
              :disabled="mobileCurrentPage === mobileTotalPages || mobileLoading"
              class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed bg-white hover:bg-gray-50 transition-colors flex items-center gap-1"
            >
              Next
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
              </svg>
            </button>
          </div>
        </div>

      </div>

      <!-- Delete Confirmation Modal -->
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-red-100 flex items-center justify-center mr-3 sm:mr-4">
              <svg class="w-5 h-5 sm:w-6 sm:h-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Subject Group</span>
          </div>
        </template>
        
        <p class="text-xs sm:text-sm text-gray-600 mt-2">
          Are you sure you want to delete this subject group? This action cannot be undone.
        </p>

        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            <Button 
              variant="secondary" 
              @click="showDeleteModal = false"
              class="w-full sm:w-auto px-4 sm:px-6 shadow-sm hover:shadow-md transition-all text-sm"
            >
              Cancel
            </Button>
            <Button 
              variant="danger" 
              @click="confirmDelete" 
              :loading="deleting"
              class="w-full sm:w-auto px-4 sm:px-6 shadow-md hover:shadow-lg transition-all text-sm"
            >
              <span v-if="!deleting">Delete Group</span>
              <span v-else>Deleting...</span>
            </Button>
          </div>
        </template>
      </Modal>

      <!-- View Subjects Modal -->
      <Modal :show="showViewModal" @close="showViewModal = false" max-width="2xl">
        <template #title>
          <div class="flex items-center">
            <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-100 flex items-center justify-center mr-3 sm:mr-4">
              <svg class="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
              </svg>
            </div>
            <div>
              <span class="text-base sm:text-lg font-semibold text-gray-900">{{ viewingGroup.group_name }}</span>
              <p v-if="viewingGroup.description" class="text-xs text-gray-500 mt-1">{{ viewingGroup.description }}</p>
            </div>
          </div>
        </template>

        <div v-if="loadingViewSubjects" class="py-8 text-center">
          <div class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600 mx-auto"></div>
        </div>

        <div v-else class="space-y-4">
          <!-- Summary -->
          <div class="grid grid-cols-3 gap-3">
            <div class="bg-blue-50 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-blue-600">{{ viewingSubjects.subjects_count?.total || 0 }}</div>
              <div class="text-xs text-gray-600 mt-1">Total</div>
            </div>
            <div class="bg-purple-50 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-purple-600">{{ viewingSubjects.subjects_count?.compulsory || 0 }}</div>
              <div class="text-xs text-gray-600 mt-1">Compulsory</div>
            </div>
            <div class="bg-gray-50 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold text-gray-600">{{ viewingSubjects.subjects_count?.optional || 0 }}</div>
              <div class="text-xs text-gray-600 mt-1">Optional</div>
            </div>
          </div>

          <!-- Compulsory Subjects -->
          <div v-if="viewingSubjects.compulsory_subjects?.length > 0">
            <h4 class="text-sm font-semibold text-purple-800 mb-2 flex items-center">
              <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
              </svg>
              Compulsory Subjects
            </h4>
            <div class="space-y-2">
              <div
                v-for="subject in viewingSubjects.compulsory_subjects"
                :key="'comp-' + subject.id"
                class="flex items-center justify-between p-3 bg-purple-50 border border-purple-200 rounded-lg"
              >
                <div>
                  <div class="font-medium text-gray-900">{{ subject.subject_name }}</div>
                  <div v-if="subject.subject_code" class="text-xs text-gray-500 font-mono">{{ subject.subject_code }}</div>
                </div>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                  Compulsory
                </span>
              </div>
            </div>
          </div>

          <!-- Optional Subjects -->
          <div v-if="viewingSubjects.optional_subjects?.length > 0">
            <h4 class="text-sm font-semibold text-gray-700 mb-2">Optional Subjects</h4>
            <div class="space-y-2">
              <div
                v-for="subject in viewingSubjects.optional_subjects"
                :key="'opt-' + subject.id"
                class="flex items-center justify-between p-3 bg-gray-50 border border-gray-200 rounded-lg"
              >
                <div>
                  <div class="font-medium text-gray-900">{{ subject.subject_name }}</div>
                  <div v-if="subject.subject_code" class="text-xs text-gray-500 font-mono">{{ subject.subject_code }}</div>
                </div>
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">
                  Optional
                </span>
              </div>
            </div>
          </div>

          <!-- Empty State -->
          <div v-if="!viewingSubjects.compulsory_subjects?.length && !viewingSubjects.optional_subjects?.length" class="text-center py-8">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No subjects in this group</p>
          </div>
        </div>

        <template #footer>
          <div class="flex justify-end">
            <Button 
              variant="secondary" 
              @click="showViewModal = false"
              class="px-4 sm:px-6 shadow-sm hover:shadow-md transition-all text-sm"
            >
              Close
            </Button>
          </div>
        </template>
      </Modal>
    </div>
  </AppLayout>
</template>

<script setup>
import { ref, onMounted, reactive, watch, computed } from 'vue'
import { router, useForm, usePage } from '@inertiajs/vue3'
import AppLayout from '@/Components/Layout/AppLayout.vue'
import Button from '@/Components/Common/Button.vue'
import Input from '@/Components/Forms/Input.vue'
import Modal from '@/Components/Common/Modal.vue'
import $ from 'jquery'
import 'datatables.net'
import axios from 'axios'

// Props from controller
const props = defineProps({
  subjects: {
    type: Array,
    default: () => []
  }
})

// State
const showDeleteModal = ref(false)
const showViewModal = ref(false)
const deleting = ref(false)
const groupToDelete = ref(null)
const tableSearch = ref('')
const perPage = ref(10)
const mobileGroups = ref([])
const mobileLoading = ref(true)
const mobileCurrentPage = ref(1)
const mobileTotalPages = ref(1)
const mobileTotal = ref(0)
const mobileOffset = ref(0)
const editMode = ref(false)
const editingId = ref(null)
const showSuccessMessage = ref(true)
const showErrorMessage = ref(true)
const allSubjects = ref([])
const loadingSubjects = ref(false)
const viewingGroup = ref({})
const viewingSubjects = ref({})
const loadingViewSubjects = ref(false)
let table = null

const filters = reactive({
  search: '',
  is_active: ''
})

// Form
const form = useForm({
  group_name: '',
  description: '',
  subject_ids: [],
  is_active: true,
})

// Watch for flash messages
const page = usePage()
watch(() => page.props.flash, (newFlash) => {
  if (newFlash.success) {
    showSuccessMessage.value = true
    setTimeout(() => {
      showSuccessMessage.value = false
    }, 10000)
  }
  if (newFlash.error) {
    showErrorMessage.value = true
    setTimeout(() => {
      showErrorMessage.value = false
    }, 10000)
  }
}, { deep: true, immediate: true })

// Computed properties
const compulsorySubjects = computed(() => {
  return allSubjects.value.filter(s => s.is_compulsory)
})

const optionalSubjects = computed(() => {
  return allSubjects.value.filter(s => !s.is_compulsory)
})

// Helper functions
const getStatusClass = (isActive) => {
  return isActive 
    ? 'bg-green-100 text-green-800' 
    : 'bg-gray-100 text-gray-800'
}

const getSubjectCount = (group) => {
  if (!group.subject_ids) return 0
  const ids = Array.isArray(group.subject_ids) ? group.subject_ids : group.subject_ids.split(',')
  return ids.length
}

const getCompulsoryCount = (group) => {
  if (!group.subject_ids) return 0
  const ids = Array.isArray(group.subject_ids) ? group.subject_ids : group.subject_ids.split(',').map(Number)
  return allSubjects.value.filter(s => ids.includes(s.id) && s.is_compulsory).length
}

const getOptionalCount = (group) => {
  return getSubjectCount(group) - getCompulsoryCount(group)
}

const getSubjectName = (subjectId) => {
  const subject = allSubjects.value.find(s => s.id === subjectId)
  return subject ? subject.subject_name : ''
}

const removeSubject = (subjectId) => {
  form.subject_ids = form.subject_ids.filter(id => id !== subjectId)
}

// Load mobile data
const loadMobileData = async () => {
  mobileLoading.value = true
  
  try {
    const params = {
      page: mobileCurrentPage.value,
      per_page: perPage.value,
      mobile: 1
    }
    
    if (filters.search) params.search = filters.search
    if (tableSearch.value) params.search = tableSearch.value
    if (filters.is_active !== '') params.is_active = filters.is_active
    
    const response = await axios.get(route('subject-groups.index'), {
      params,
      headers: {
        'X-Requested-With': 'XMLHttpRequest',
        'Accept': 'application/json'
      }
    })
    
    if (response.data) {
      if (response.data.data) {
        mobileGroups.value = response.data.data
        mobileCurrentPage.value = response.data.current_page || 1
        mobileTotalPages.value = response.data.last_page || 1
        mobileTotal.value = response.data.total || 0
        mobileOffset.value = response.data.from ? response.data.from - 1 : 0
      } else if (Array.isArray(response.data)) {
        mobileGroups.value = response.data
        mobileTotalPages.value = 1
        mobileTotal.value = response.data.length
        mobileOffset.value = 0
      }
    }
  } catch (error) {
    console.error('Error loading mobile data:', error)
    mobileGroups.value = []
    mobileTotal.value = 0
  } finally {
    mobileLoading.value = false
  }
}

// Initialize on mount
onMounted(() => {
  // ✅ Set subjects from props instead of API call
  allSubjects.value = props.subjects || []
  loadingSubjects.value = false
  
  loadMobileData()

  // Initialize desktop table
  table = $('#subject-groups-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: route('subject-groups.index'),
      data: function(d) {
        d.search.value = filters.search || tableSearch.value
        if (filters.is_active !== '') {
          d.is_active = filters.is_active
        }
      }
    },
    columns: [
      { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
      { data: 'group_name', name: 'group_name' },
      { data: 'description', name: 'description' },
      { data: 'subjects_count', name: 'subjects_count', orderable: false },
      { data: 'is_active', name: 'is_active' },
      { data: 'action', name: 'action', orderable: false, searchable: false, className: 'text-center' }
    ],
    pageLength: 10,
    lengthMenu: [[10, 25, 50, 100], [10, 25, 50, 100]],
    order: [[1, 'asc']],
    searching: true,
    info: true,
    responsive: true,
    
    dom: '<"flex items-center justify-between border-b border-gray-200"<"ml-auto"i>>rt<"flex items-center justify-between px-6 py-4 border-t border-gray-200"<"text-sm text-gray-600"i>p>',
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg><p class="mt-2 text-sm font-medium">No groups found</p></div>',
      info: 'Showing _START_ to _END_ of _TOTAL_ entries',
      infoEmpty: 'Showing 0 to 0 of 0 entries',
      infoFiltered: '(filtered from _MAX_ total entries)',
      processing: '<div class="flex items-center justify-center py-8"><div class="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div></div>',
      paginate: {
        first: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7"/></svg>',
        last: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"/></svg>',
        next: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>',
        previous: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>'
      }
    },
    drawCallback: function () {
      const info = $('#subject-groups-table_info')
      $('#table-info').empty().append(info)

      const paginate = $('#subject-groups-table_paginate')
      $('#table-pagination').empty().append(paginate)
    }
  })
})

// Form submit
const submit = () => {
  if (editMode.value) {
    form.put(route('subject-groups.update', editingId.value), {
      preserveScroll: true,
      onSuccess: () => {
        resetForm()
        loadData()
      }
    })
  } else {
    form.post(route('subject-groups.store'), {
      preserveScroll: true,
      onSuccess: () => {
        resetForm()
        loadData()
      }
    })
  }
}

// Reset form
const resetForm = () => {
  form.reset()
  form.clearErrors()
  editMode.value = false
  editingId.value = null
}

// Cancel edit
const cancelEdit = () => {
  resetForm()
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

// Edit subject group (called from DataTables)
window.editSubjectGroup = (groupData) => {
  editMode.value = true
  editingId.value = groupData.id
  
  form.group_name = groupData.group_name
  form.description = groupData.description
  form.subject_ids = Array.isArray(groupData.subject_ids) ? groupData.subject_ids : []
  form.is_active = groupData.is_active
  
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

// Edit subject group mobile
const editSubjectGroupMobile = (group) => {
  editMode.value = true
  editingId.value = group.id
  
  form.group_name = group.group_name
  form.description = group.description
  
  // Convert subject_ids to array
  if (group.subject_ids) {
    form.subject_ids = Array.isArray(group.subject_ids) 
      ? group.subject_ids 
      : group.subject_ids.split(',').map(Number)
  } else {
    form.subject_ids = []
  }
  
  form.is_active = group.is_active
  
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

// View group subjects
const viewGroupSubjectsMobile = async (groupId) => {
  showViewModal.value = true
  loadingViewSubjects.value = true
  
  try {
    const response = await axios.get(route('subject-groups.group-subjects', groupId))
    viewingGroup.value = response.data.group
    viewingSubjects.value = response.data
  } catch (error) {
    console.error('Error loading group subjects:', error)
  } finally {
    loadingViewSubjects.value = false
  }
}

window.viewGroupSubjects = viewGroupSubjectsMobile

// Delete subject group
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('subject-groups.destroy', groupToDelete.value), {
    onSuccess: () => {
      showDeleteModal.value = false
      deleting.value = false
      loadData()
    },
    onError: () => {
      deleting.value = false
    }
  })
}

window.deleteSubjectGroup = (id) => {
  groupToDelete.value = id
  showDeleteModal.value = true
}

// Mobile pagination
const prevPage = () => {
  if (mobileCurrentPage.value > 1 && !mobileLoading.value) {
    mobileCurrentPage.value--
    loadMobileData()
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
}

const nextPage = () => {
  if (mobileCurrentPage.value < mobileTotalPages.value && !mobileLoading.value) {
    mobileCurrentPage.value++
    loadMobileData()
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }
}

// Table search with debounce
let tableSearchTimeout = null
const tableSearchDebounced = () => {
  clearTimeout(tableSearchTimeout)
  tableSearchTimeout = setTimeout(() => {
    loadData()
  }, 500)
}

// Filter search with debounce
let searchTimeout = null
const searchDebounced = () => {
  clearTimeout(searchTimeout)
  searchTimeout = setTimeout(() => {
    loadData()
  }, 500)
}

// Change per page
const changePerPage = () => {
  if (table) {
    table.page.len(perPage.value).draw()
  }
  mobileCurrentPage.value = 1
  loadMobileData()
}

// Reload table
const loadData = () => {
  if (table) {
    table.ajax.reload()
  }
  mobileCurrentPage.value = 1
  loadMobileData()
}

// Reset filters
const resetFilters = () => {
  filters.search = ''
  filters.is_active = ''
  tableSearch.value = ''
  loadData()
}
</script>

<style scoped>
:deep(.dataTables_info) {
  font-size: 0.875rem;
  color: #4b5563;
  font-weight: 500;
}

:deep(.dataTables_paginate) {
  display: flex;
  justify-content: flex-end;
  gap: 0.25rem;
  flex-wrap: wrap;
}

:deep(.paginate_button) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
  font-weight: 500;
  border: 1px solid #d1d5db;
  border-radius: 0.5rem;
  background: white;
  color: #374151;
  cursor: pointer;
  transition: all 0.2s;
}

:deep(.paginate_button:hover:not(.disabled)) {
  background: #f3f4f6;
  border-color: #9ca3af;
}

:deep(.paginate_button.current) {
  background: #2563eb;
  color: white;
  border-color: #2563eb;
}

:deep(.paginate_button.current:hover) {
  background: #1d4ed8;
  border-color: #1d4ed8;
}

:deep(.paginate_button.disabled) {
  opacity: 0.5;
  cursor: not-allowed;
  background: #f9fafb;
}

:deep(#subject-groups-table_info),
:deep(#subject-groups-table_paginate) {
  display: none;
}

#table-info :deep(.dataTables_info),
#table-pagination :deep(.dataTables_paginate) {
  display: block;
}

:deep(#subject-groups-table tbody td) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
}

@media (min-width: 640px) {
  :deep(#subject-groups-table tbody td) {
    padding: 0.75rem 1.5rem;
    font-size: 0.875rem;
  }
}

@media (max-width: 1024px) {
  :deep(#subject-groups-table) {
    font-size: 0.813rem;
  }
  
  :deep(#subject-groups-table th),
  :deep(#subject-groups-table td) {
    padding: 0.5rem;
  }
}
</style>