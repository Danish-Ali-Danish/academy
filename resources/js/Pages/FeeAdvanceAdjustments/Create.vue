    ...
    <div class="min-h-screen flex flex-col">
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Advance Adjustment</h1>
            <p class="mt-2 text-sm text-gray-600">Apply available advance payment directly to a pending voucher.</p>
          </div>
          <Button @click="$inertia.visit(route('fee-advance-adjustments.index'))" variant="secondary" class="w-full sm:w-auto">
            Back to List
          </Button>
        </div>
        ...
        <form @submit.prevent="submit" class="bg-white rounded-xl shadow-md p-4 sm:p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <SearchablePicker
                v-model="form.from_payment_id"
                :items="advancePayments"
                title="Available Advance Payments"
                placeholder="Search receipt, student, admission..."
                empty-text="No available advance payments found"
                :error="form.errors.from_payment_id"
            ...
            <div>
              ...
              <SearchablePicker
                v-model="form.to_voucher_id"
                :items="filteredVouchers"
                title="Pending Vouchers"
                placeholder="Search voucher, student, fee type..."
                empty-text="No matching pending vouchers found"
                :error="form.errors.to_voucher_id"
                @select="selectVoucher"
                @clear="clearVoucher"
              />
            </div>

            <div v-if="selectedAdvance || selectedVoucher" class="md:col-span-2 rounded-lg border border-indigo-100 bg-indigo-50 p-4">
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  <p class="text-xs font-semibold uppercase tracking-wide text-indigo-400">Will Create Payment</p>
                  <p class="mt-1 font-semibold text-gray-900">Method: Advance Adjusted</p>
                </div>
              </div>
            </div>
            ...
            <div class="md:col-span-2">
              ...
              <textarea v-model="form.notes" rows="3" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Optional adjustment note..." />
            </div>
          </div>
          ...
          <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3">
            ...
            <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto">
              <span v-if="!form.processing">Apply Advance</span>
...
const maxAdjustable = computed(() => {
  if (!selectedAdvance.value || !selectedVoucher.value) return 0
  return Math.min(Number(selectedAdvance.value.available_amount || 0), Number(selectedVoucher.value.remaining_amount || 0))
})
...
const selectVoucher = () => autoAmount()
...
const submit = () => {
  form.post(route('fee-advance-adjustments.store'), { preserveScroll: true })
}
</script>
