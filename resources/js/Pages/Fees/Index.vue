      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Fee Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage student fee records and payments</p>
            </div>
            <Link :href="route('fees.create')">
              <Button variant="primary" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all duration-200">
                ...
                <span class="text-sm sm:text-base">Create Fee Record</span>
              </Button>
            </Link>
          </div>
        </div>
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="fees-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Month/Year</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Class</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Total</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Paid</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Balance</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileFees.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No fee records found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Fee Cards -->
          <div v-else v-for="(fee, index) in mobileFees" :key="fee.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ fee.student ? fee.student.full_name : 'N/A' }}</h3>
                  </div>
                  <p class="text-xs text-gray-500 mt-0.5">{{ fee.student ? fee.student.admission_no : 'N/A' }}</p>
                </div>
                <span :class="getStatusClass(fee.status)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ fee.fee_type ? fee.fee_type.name : 'N/A' }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ formatMonthYear(fee.month, fee.year) }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ fee.class ? fee.class.name : 'N/A' }}</span>
                </div>

                <div class="grid grid-cols-3 gap-2 pt-2 border-t border-gray-100">
                  <div class="text-center">
                    <p class="text-xs text-gray-500">Total</p>
                    <p class="text-sm font-semibold text-gray-900">Rs. {{ Number(fee.total_amount).toLocaleString() }}</p>
                  </div>
                  <div class="text-center">
                    <p class="text-xs text-gray-500">Paid</p>
                    <p class="text-sm font-semibold text-green-600">Rs. {{ Number(fee.paid_amount).toLocaleString() }}</p>
                  </div>
                  <div class="text-center">
                    <p class="text-xs text-gray-500">Balance</p>
                    <p class="text-sm font-semibold text-red-600">Rs. {{ Number(fee.balance_amount).toLocaleString() }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <Link :href="route('fees.edit', fee.id)" class="flex-1">
                ...
                <button 
                  @click="() => { feeToDelete = fee.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileFees.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total fees</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Fee Record</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Fee</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('fees.index'), {
      params,
      headers: {
    ...
    if (response.data) {
      if (response.data.data) {
        mobileFees.value = response.data.data
        ...
        mobileFees.value = response.data
  ...
  table = $('#fees-table').DataTable({
    ...
    ajax: {
      ...
      data: function(d) {
        ...
        d.fee_type_id = filters.fee_type_id
      }
    },
    columns: [
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p class="mt-2 text-sm font-medium">No fee records found</p></div>',
    ...
    drawCallback: function () {
      const info = $('#fees-table_info')
      $('#table-info').empty().append(info)

      const paginate = $('#fees-table_paginate')
...
:deep(#fees-table_info),
:deep(#fees-table_paginate) {
  display: none;
}
