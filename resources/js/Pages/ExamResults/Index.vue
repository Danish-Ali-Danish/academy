      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-sm text-slate-600 flex items-center gap-2">
                ...
                Manage student examination results and grades
              </p>
            </div>
            <Link :href="route('exam-results.create')">
              <Button variant="primary" class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-500/30 hover:shadow-xl hover:shadow-blue-500/40 transition-all duration-200">
                ...
                <span class="text-sm font-semibold">Enter Results</span>
              </Button>
            </Link>
          </div>
        </div>
        ...
        <div class="hidden md:block bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="results-table" class="min-w-full divide-y divide-slate-100">
              <thead class="bg-gradient-to-r from-slate-50 via-blue-50/30 to-indigo-50/20">
                <tr>
                  ...
                  <th class="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Obtained</th>
        ...
        <div class="md:hidden space-y-4">
          ...
          <div v-else-if="mobileData.length === 0" class="text-center py-12 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-slate-100">
            ...
            <p class="mt-2 text-sm font-medium text-slate-500">No results found</p>
            <p class="mt-1 text-xs text-slate-400">Try adjusting your filters</p>
          </div>

          <!-- Result Cards -->
          <div v-else v-for="(result, index) in mobileData" :key="result.id" class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-slate-200/60 transition-all duration-300">
            <div class="p-5">
              <!-- Header -->
              <div class="flex items-start justify-between mb-4">
                <div class="flex-1">
                  ...
                  <h3 class="text-base font-bold text-slate-900">{{ result.student.first_name }} {{ result.student.last_name }}</h3>
                  <p class="text-xs text-slate-500 font-mono mt-0.5">Roll: {{ result.student.roll_number }}</p>
                </div>
                <span :class="getGradeClass(result.grade)" class="px-3 py-1.5 text-sm font-bold rounded-lg whitespace-nowrap ml-2 shadow-sm">
                  {{ result.grade || 'N/A' }}
                </span>
              </div>
              ...
              <div class="space-y-3 border-t border-slate-100 pt-4">
                <div class="flex items-center gap-3 text-sm">
                  ...
                  <span class="text-slate-700 font-medium">{{ result.exam.name }}</span>
                </div>
                ...
                <div class="flex items-center gap-3 text-sm">
                  ...
                  <span class="text-slate-700">{{ result.subject.name }}</span>
                </div>

                <div class="grid grid-cols-3 gap-3">
                  <div class="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-3 text-center border border-blue-200/50">
                    <p class="text-xs text-blue-600 font-medium">Obtained</p>
                    <p class="text-lg font-bold text-blue-700 mt-1">{{ result.obtained_marks }}</p>
                  </div>
                  <div class="bg-gradient-to-br from-purple-50 to-purple-100/50 rounded-xl p-3 text-center border border-purple-200/50">
                    <p class="text-xs text-purple-600 font-medium">Total</p>
                    <p class="text-lg font-bold text-purple-700 mt-1">{{ result.total_marks }}</p>
                  </div>
                  <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-xl p-3 text-center border border-emerald-200/50">
                    <p class="text-xs text-emerald-600 font-medium">Score</p>
                    <p class="text-lg font-bold text-emerald-700 mt-1">{{ result.percentage }}%</p>
                  </div>
                </div>

                <div class="flex justify-center pt-2">
                  <span :class="result.is_pass ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-red-100 text-red-700 border-red-200'" class="px-4 py-1.5 text-sm font-semibold rounded-full border inline-flex items-center gap-2">
                    <span v-if="result.is_pass">✓</span>
                    <span v-else>✗</span>
              ...
              <div class="flex gap-3 mt-5 pt-4 border-t border-slate-100">
                <Link :href="route('exam-results.edit', result.id)" class="flex-1">
                ...
                <button 
                  @click="() => { itemToDelete = result.id; showDeleteModal = true; }"
        ...
        <div v-if="mobileData.length > 0" class="md:hidden mt-4 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 p-4">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-slate-500 mt-0.5">{{ mobileTotal }} total results</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-lg font-semibold text-slate-900">Delete Result</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-3">
            ...
            >
              <span v-if="!deleting">Delete Result</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    if (filters.pass_status) params.pass_status = filters.pass_status
    
    const response = await axios.get(route('exam-results.index'), {
      params,
  ...
  table = $('#results-table').DataTable({
    ...
    ajax: {
      ...
      data: function(d) {
        ...
        d.pass_status = filters.pass_status
      },
    ...
    drawCallback: function () {
      $('#table-info').empty().append($('#results-table_info'))
      $('#table-pagination').empty().append($('#results-table_paginate'))
    }
  })
})
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('exam-results.destroy', itemToDelete.value), {
    onSuccess: () => {
...
:deep(#results-table_info),
:deep(#results-table_paginate) {
  display: none;
}
