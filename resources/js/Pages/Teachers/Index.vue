      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Teacher Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage all teaching staff</p>
            </div>
            <Link :href="route('teachers.create')">
              <Button variant="primary" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all duration-200">
                ...
                <span class="text-sm sm:text-base">Add New Teacher</span>
              </Button>
            </Link>
          </div>
        </div>
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="teachers-table" class="min-w-full divide-y divide-gray-200">
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileTeachers.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No teachers found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Teacher Cards -->
          <div v-else v-for="(teacher, index) in mobileTeachers" :key="teacher.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  ...
                  <p class="text-xs text-gray-500 mt-0.5">ID: {{ teacher.employee_id }}</p>
                </div>
                <span :class="getStatusClass(teacher.status)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ teacher.phone || 'N/A' }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ teacher.email || 'N/A' }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ teacher.qualification || 'N/A' }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600 font-medium">{{ teacher.total_salary }}</span>
                </div>
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">{{ teacher.branch || 'N/A' }}</span>
                </div>
              </div>

              <!-- Actions -->
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <Link :href="route('teachers.edit', teacher.id)" class="flex-1">
                ...
                <button 
                  @click="() => { teacherToDelete = teacher.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileTeachers.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total teachers</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Teacher</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Teacher</span>
...
const statusOptions = ['active', 'inactive', 'on_leave', 'resigned']
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('teachers.index'), {
      params,
      headers: {
    ...
    if (response.data) {
      if (response.data.data) {
        mobileTeachers.value = response.data.data
        ...
        mobileTeachers.value = response.data
...
onMounted(() => {
  ...
  table = $('#teachers-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
    ...
    drawCallback: function () {
      const info = $('#teachers-table_info')
      $('#table-info').empty().append(info)

      const paginate = $('#teachers-table_paginate')
      $('#table-pagination').empty().append(paginate)
    }
  })

  $('#teachers-table').on('click', '[data-delete]', function() {
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('teachers.destroy', teacherToDelete.value), {
    onSuccess: () => {
...
:deep(#teachers-table_info),
:deep(#teachers-table_paginate) {
  display: none;
}
...
@media (max-width: 1024px) {
  ...
  :deep(#teachers-table th),
  :deep(#teachers-table td) {
    padding: 0.5rem;
  }
}
</style>