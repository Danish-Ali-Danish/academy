      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Subjects Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage all curriculum subjects</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  required
                />
                <p v-if="form.errors.subject_name" class="mt-1 text-sm text-red-600">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                  :class="{ 'border-red-500': form.errors.subject_code }"
                  placeholder="e.g., MATH-101, ENG-201"
                />
                <p v-if="form.errors.subject_code" class="mt-1 text-sm text-red-600">
              ...
              <div>
                ...
                <p v-if="form.errors.is_compulsory" class="mt-1 text-sm text-red-600">
                  {{ form.errors.is_compulsory }}
                </p>
              </div>
            ...
            <div class="mt-6 flex items-center justify-end gap-3 sm:gap-4">
              ...
              >
                <span v-if="!form.processing">{{ editMode ? 'Update Subject' : 'Create Subject' }}</span>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            ...
            <div>
              <select
                v-model="filters.is_compulsory"
                @change="loadData"
                class="w-full px-3 sm:px-4 py-2 text-sm border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              >
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileSubjects.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No subjects found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Subject Cards -->
          <div v-else v-for="(subject, index) in mobileSubjects" :key="subject.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ subject.subject_name }}</h3>
                  </div>
                  <p v-if="subject.subject_code" class="text-xs text-gray-500 mt-0.5 font-mono">{{ subject.subject_code }}</p>
                </div>
                <span :class="getStatusClass(subject.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <!-- Compulsory Badge -->
              <div class="mb-3">
                <span v-if="subject.is_compulsory" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button 
                  @click="editSubjectMobile(subject)"
                ...
                <button 
                  @click="() => { subjectToDelete = subject.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileSubjects.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            <button 
              @click="prevPage"
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total subjects</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Subject</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Subject</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    if (filters.is_compulsory !== '') params.is_compulsory = filters.is_compulsory
    
    const response = await axios.get(route('subjects.index'), {
      params,
      headers: {
    ...
    if (response.data) {
      if (response.data.data) {
        mobileSubjects.value = response.data.data
        ...
        mobileSubjects.value = response.data
  ...
  table = $('#subjects-table').DataTable({
    ...
    ajax: {
      ...
      data: function(d) {
        ...
        if (filters.is_compulsory !== '') {
          d.is_compulsory = filters.is_compulsory
        }
      }
    },
    columns: [
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p class="mt-2 text-sm font-medium">No subjects found</p></div>',
    ...
    drawCallback: function () {
      const info = $('#subjects-table_info')
      $('#table-info').empty().append(info)

      const paginate = $('#subjects-table_paginate')
...
const submit = () => {
  if (editMode.value) {
    form.put(route('subjects.update', editingId.value), {
      preserveScroll: true,
      onSuccess: () => {
        resetForm()
        loadData()
...
// Edit subject (called from DataTables)
window.editSubject = (subjectData) => {
  editMode.value = true
  editingId.value = subjectData.id
  
  form.subject_name = subjectData.subject_name
  form.subject_code = subjectData.subject_code
  form.is_compulsory = subjectData.is_compulsory
  form.is_active = subjectData.is_active
  
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

// Edit subject mobile
const editSubjectMobile = (subject) => {
  editMode.value = true
  editingId.value = subject.id
  
  form.subject_name = subject.subject_name
  form.subject_code = subject.subject_code
  form.is_compulsory = subject.is_compulsory
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('subjects.destroy', subjectToDelete.value), {
    onSuccess: () => {
...
:deep(#subjects-table_info),
:deep(#subjects-table_paginate) {
  display: none;
}
...
@media (max-width: 1024px) {
  ...
  :deep(#subjects-table th),
  :deep(#subjects-table td) {
    padding: 0.5rem;
  }
}
</style>