      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Section Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage all academy sections</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Section A, Section B, Morning"
                  required
                />
                <p v-if="form.errors.section_name" class="mt-1 text-sm text-red-600">
            ...
            <div class="mt-6 flex items-center justify-end gap-3 sm:gap-4">
              ...
              >
                <span v-if="!form.processing">{{ editMode ? 'Update Section' : 'Create Section' }}</span>
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="sections-table" class="min-w-full divide-y divide-gray-200">
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileSections.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No sections found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Section Cards -->
          <div v-else v-for="(section, index) in mobileSections" :key="section.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                ...
                <span :class="getStatusClass(section.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center justify-between text-xs sm:text-sm">
                  ...
                  <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                    {{ section.class_sections_count || 0 }} classes
                  </span>
                </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button 
                  @click="editSectionMobile(section)"
                ...
                <button 
                  @click="() => { sectionToDelete = section.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileSections.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            <button 
              @click="prevPage"
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Section</span>
          </div>
        </template>
        
        <p class="text-xs sm:text-sm text-gray-600 mt-2">
          Are you sure you want to delete this section? This action cannot be undone and will affect all associated classes and students.
        </p>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('sections.index'), {
      params,
      headers: {
    ...
    if (response.data) {
      if (response.data.data) {
        mobileSections.value = response.data.data
        ...
        mobileSections.value = response.data
...
onMounted(() => {
  ...
  table = $('#sections-table').DataTable({
    ...
    columns: [
      ...
      { data: 'class_sections_count', name: 'class_sections_count', orderable: false, searchable: false },
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg><p class="mt-2 text-sm font-medium">No sections found</p></div>',
    ...
    drawCallback: function () {
      const info = $('#sections-table_info')
      $('#table-info').empty().append(info)

      const paginate = $('#sections-table_paginate')
...
const submit = () => {
  if (editMode.value) {
    form.put(route('sections.update', editingId.value), {
      preserveScroll: true,
      onSuccess: () => {
        resetForm()
        loadData()
...
// Edit section (called from DataTables)
window.editSection = (sectionData) => {
  editMode.value = true
  editingId.value = sectionData.id
  
  form.section_name = sectionData.section_name
  form.is_active = sectionData.is_active
  
  window.scrollTo({ top: 0, behavior: 'smooth' })
}

// Edit section mobile
const editSectionMobile = (section) => {
  editMode.value = true
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('sections.destroy', sectionToDelete.value), {
    onSuccess: () => {
...
:deep(#sections-table_info),
:deep(#sections-table_paginate) {
  display: none;
}
...
@media (max-width: 1024px) {
  ...
  :deep(#sections-table th),
  :deep(#sections-table td) {
    padding: 0.5rem;
  }
}
</style>