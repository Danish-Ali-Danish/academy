<?php

namespace App\Http\Controllers;

use App\Models\VoucherDiscountBreakdown;
use App\Models\FeeVoucher;
use App\Models\FeeWaiver;
use Illuminate\Http\Request;
use Inertia\Inertia;

class VoucherDiscountBreakdownController extends Controller
{
    /**
     * Read-only controller — discount breakdowns are system-generated
     */
    public function index(Request $request)
    {
        if ($request->has('mobile') || ($request->ajax() && $request->get('page'))) { return $this->getMobileBreakdowns($request); }
        if ($request->ajax() && $request->has('draw')) { return $this->getDataTablesBreakdowns($request); }

        $selectedVoucher = null;
        if ($request->filled('voucher_id')) {
            $selectedVoucher = FeeVoucher::with(['studentEnrollment.student', 'feeType'])
                ->find($request->voucher_id);
        }

        $selectedWaivers = collect();
        $approvedWaiverAmount = 0;

        if ($selectedVoucher) {
            $selectedWaivers = FeeWaiver::with(['approvedBy', 'reversedBy'])
                ->where('voucher_id', $selectedVoucher->id)
                ->latest()
                ->get();

            $approvedWaiverAmount = (float) $selectedWaivers
                ->filter(fn ($waiver) => strtolower((string) $waiver->status) === 'approved')
                ->sum(fn ($waiver) => (float) $waiver->waived_amount);
        }

        return Inertia::render('VoucherDiscountBreakdowns/Index', [
            'voucherId' => $request->query('voucher_id'),
            'selectedVoucher' => $selectedVoucher ? [
                'id' => $selectedVoucher->id,
                'voucher_no' => $selectedVoucher->voucher_no,
                'student_name' => $selectedVoucher->studentEnrollment?->student?->student_name ?? '-',
                'fee_type' => $selectedVoucher->feeType?->fee_name ?? '-',
                'original_amount' => $selectedVoucher->original_amount,
                'discount_amount' => $selectedVoucher->discount_amount,
                'fine_amount' => $selectedVoucher->fine_amount,
                'waiver_amount' => $approvedWaiverAmount,
                'net_amount' => $selectedVoucher->net_amount,
                'waivers' => $selectedWaivers->map(fn ($waiver) => [
                    'id' => $waiver->id,
                    'waived_amount' => $waiver->waived_amount,
                    'waiver_reason' => $waiver->waiver_reason,
                    'approved_by' => $waiver->approvedBy?->name ?? '-',
                    'approved_on' => $waiver->approved_on?->format('d M, Y') ?? '-',
                    'status' => strtolower((string) $waiver->status),
                    'reversal_reason' => $waiver->reversal_reason,
                    'reversed_by' => $waiver->reversedBy?->name ?? '-',
                ])->values(),
            ] : null,
        ]);
    }

    public function show(VoucherDiscountBreakdown $voucherDiscountBreakdown)
    {
        $voucherDiscountBreakdown->load(['voucher.studentEnrollment.student', 'voucher.feeType', 'voucher.academicYear', 'appliedBy']);
        $waivers = FeeWaiver::with(['approvedBy', 'reversedBy'])
            ->where('voucher_id', $voucherDiscountBreakdown->voucher_id)
            ->latest()
            ->get();

        return Inertia::render('VoucherDiscountBreakdowns/Show', [
            'breakdown' => $voucherDiscountBreakdown,
            'waiverAmount' => (float) $waivers
                ->filter(fn ($waiver) => strtolower((string) $waiver->status) === 'approved')
                ->sum(fn ($waiver) => (float) $waiver->waived_amount),
            'waivers' => $waivers->map(fn ($waiver) => [
                'id' => $waiver->id,
                'waived_amount' => $waiver->waived_amount,
                'waiver_reason' => $waiver->waiver_reason,
                'approved_by' => $waiver->approvedBy?->name ?? '-',
                'approved_on' => $waiver->approved_on?->format('d M, Y') ?? '-',
                'status' => strtolower((string) $waiver->status),
                'reversal_reason' => $waiver->reversal_reason,
                'reversed_by' => $waiver->reversedBy?->name ?? '-',
            ])->values(),
        ]);
    }

    private function getMobileBreakdowns(Request $request)
    {
        $query = VoucherDiscountBreakdown::with(['voucher.studentEnrollment.student', 'voucher.feeType', 'appliedBy']);
        if ($request->filled('search')) {
            $search = $request->search;
            $query->where(function ($q) use ($search) {
                $q->where('discount_source', 'like', "%{$search}%")
                    ->orWhere('source_label', 'like', "%{$search}%")
                    ->orWhere('calculated_amount', 'like', "%{$search}%")
                    ->orWhereHas('voucher', fn ($voucherQuery) => $voucherQuery->where('voucher_no', 'like', "%{$search}%"))
                    ->orWhereHas('voucher.studentEnrollment.student', fn ($studentQuery) => $studentQuery->where('student_name', 'like', "%{$search}%")
                        ->orWhere('roll_no', 'like', "%{$search}%")
                        ->orWhere('admission_no', 'like', "%{$search}%"));
            });
        }
        if ($request->filled('voucher_id')) { $query->where('voucher_id', $request->voucher_id); }
        if ($request->filled('source')) { $query->where('discount_source', $request->source); }
        return response()->json($query->latest()->paginate($request->get('per_page', 10)));
    }

    private function getDataTablesBreakdowns(Request $request)
    {
        $query = VoucherDiscountBreakdown::with(['voucher.studentEnrollment.student', 'voucher.feeType', 'appliedBy']);
        if ($request->filled('search.value')) {
            $search = $request->input('search.value');
            $query->where(function ($q) use ($search) {
                $q->where('discount_source', 'like', "%{$search}%")
                    ->orWhere('source_label', 'like', "%{$search}%")
                    ->orWhere('calculated_amount', 'like', "%{$search}%")
                    ->orWhereHas('voucher', fn ($voucherQuery) => $voucherQuery->where('voucher_no', 'like', "%{$search}%"))
                    ->orWhereHas('voucher.studentEnrollment.student', fn ($studentQuery) => $studentQuery->where('student_name', 'like', "%{$search}%")
                        ->orWhere('roll_no', 'like', "%{$search}%")
                        ->orWhere('admission_no', 'like', "%{$search}%"));
            });
        }
        if ($request->filled('voucher_id')) { $query->where('voucher_id', $request->voucher_id); }
        if ($request->filled('source')) { $query->where('discount_source', $request->source); }

        $recordsFiltered = (clone $query)->count();
        $columns = ['id', 'voucher_id', 'discount_source', 'calculated_amount', 'created_at'];
        $orderColumn = $request->input('order.0.column', 0); $orderDir = $request->input('order.0.dir', 'desc');
        if (isset($columns[$orderColumn])) { $query->orderBy($columns[$orderColumn], $orderDir); }

        $start = $request->input('start', 0); $length = $request->input('length', 10);
        $breakdowns = $query->skip($start)->take($length)->get();

        $data = $breakdowns->map(function ($b, $index) use ($start) {
            $sourceLabel = $b->source_label ?: ucwords(str_replace('_', ' ', $b->discount_source ?? '-'));
            $student = $b->voucher?->studentEnrollment?->student;
            $discountSourceClass = match ($b->discount_source) {
                'student_fee_concession' => 'bg-green-100 text-green-800',
                'sibling_discount_rule' => 'bg-blue-100 text-blue-800',
                'student_scholarship' => 'bg-purple-100 text-purple-800',
                default => 'bg-gray-100 text-gray-800',
            };

            return [
                'DT_RowIndex' => $start + $index + 1, 'id' => $b->id,
                'voucher_no' => $b->voucher?->voucher_no ?? '-',
                'student' => $student?->student_name ?? '-',
                'fee_type' => $b->voucher?->feeType?->fee_name ?? '-',
                'discount_source' => '<span class="px-2 py-1 text-xs font-medium rounded-full ' . $discountSourceClass . '">' . e($sourceLabel) . '</span>',
                'discount_type' => ucfirst($b->discount_type ?? '-'),
                'discount_value' => $b->discount_type === 'percentage' ? ($b->discount_value ?? 0) . '%' : number_format($b->discount_value ?? 0, 2),
                'discount_amount' => number_format($b->calculated_amount, 2),
                'applied_by' => $b->appliedBy?->name ?? 'System',
                'created_at' => $b->created_at?->format('d M, Y H:i') ?? '-',
                'action' => '<div class="flex items-center justify-center"><button onclick=\'viewBreakdown(' . json_encode(['id' => $b->id]) . ')\' class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>View</button></div>'
            ];
        });
        return response()->json([
            'draw' => intval($request->input('draw')),
            'recordsTotal' => VoucherDiscountBreakdown::count(),
            'recordsFiltered' => $recordsFiltered,
            'data' => $data,
        ]);
    }
}
